...
<div x-data="{ activeTab: 'registration', showLogModal: false, activeLogs: [] }" class="space-y-6">
    ...
    <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
        ...
        <p class="text-xs text-slate-500 mt-0.5">Alur sekuensial 3-Layer Approval (PIC Layer 1 &rarr; Section Head Layer 2 &rarr; Department Head Layer 3). Persetujuan Dept Head mengeksekusi sistem secara otomatis.</p>
    </div>
    ...
    <div class="flex space-x-2 border-b border-slate-300">
        <button @click="activeTab = 'registration'" 
                ...
                class="px-4 py-3 rounded-t-xl border-b-2 text-xs transition flex items-center gap-2">
    ...
    <div x-show="activeTab === 'registration'" x-cloak class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <h4 class="text-xs font-bold text-slate-900 uppercase tracking-wider border-b pb-2 mb-4">Pengajuan Pendaftaran Dokumen Baru (Draft Upload &rarr; Master Relocation)</h4>
        <form action="{{ route('lifecycle.registration') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            ...
            <div>
                ...
                <input type="text" name="doc_number" required placeholder="SOP-K3-2026-001..." class="w-full p-2.5 border border-slate-300 rounded-xl">
            </div>
            <div>
                <label class="block font-bold text-slate-700 mb-1">Judul Dokumen:</label>
                <input type="text" name="title" required placeholder="SOP Keselamatan Kerja Listrik..." class="w-full p-2.5 border border-slate-300 rounded-xl">
            </div>
            <div>
                <label class="block font-bold text-slate-700 mb-1">Departemen Pemilik:</label>
                <select name="department" required class="w-full p-2.5 border border-slate-300 rounded-xl">
            ...
            <div>
                ...
                <select name="category" required class="w-full p-2.5 border border-slate-300 rounded-xl">
            ...
            <div class="md:col-span-2">
                ...
                <input type="file" name="document_file" accept="application/pdf" required class="w-full p-2.5 border border-slate-300 rounded-xl bg-slate-50">
            </div>
            <div class="md:col-span-2">
                <label class="block font-bold text-slate-700 mb-1">Alasan Penerbitan Dokumen Baru:</label>
                <textarea name="reason" required rows="3" placeholder="Jelaskan latar belakang penerbitan dokumen..." class="w-full p-2.5 border border-slate-300 rounded-xl"></textarea>
            </div>
            <div class="md:col-span-2 text-right">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-6 py-2.5 rounded-xl shadow transition">
    ...
    <div x-show="activeTab === 'revision'" x-cloak class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <h4 class="text-xs font-bold text-slate-900 uppercase tracking-wider border-b pb-2 mb-4">Pengajuan Revisi Dokumen (Meningkatkan Revision Rev 00 &rarr; Rev 01)</h4>
        <form action="{{ route('lifecycle.revision') }}" method="POST" enctype="multipart/form-data" class="space-y-4 text-xs">
            ...
            <div>
                ...
                <select name="target_document_id" required class="w-full p-2.5 border border-slate-300 rounded-xl">
            ...
            <div>
                ...
                <textarea name="reason" required rows="3" placeholder="Jelaskan perubahan klausul/pasal pada dokumen revisi..." class="w-full p-2.5 border border-slate-300 rounded-xl"></textarea>
            </div>
    ...
    <div x-show="activeTab === 'obsolete'" x-cloak class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <h4 class="text-xs font-bold text-slate-900 uppercase tracking-wider border-b pb-2 mb-4">Pengajuan Penarikan Dokumen Kadaluwarsa (Obsolete)</h4>
        <form action="{{ route('lifecycle.obsolete') }}" method="POST" class="space-y-4 text-xs">
            ...
            <div>
                ...
                <textarea name="reason" required rows="3" placeholder="Jelaskan alasan penarikan/kadaluwarsa dokumen..." class="w-full p-2.5 border border-slate-300 rounded-xl"></textarea>
            </div>
            <div class="text-right">
                <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-bold px-6 py-2.5 rounded-xl shadow transition">
    ...
    <!-- Status Tracking & Approval Progress Table -->
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="p-4 bg-slate-900 text-white flex justify-between items-center">
            ...
            <span class="text-[10px] bg-slate-800 text-slate-300 px-2 py-0.5 rounded font-mono">Sequential Workflow</span>
        </div>
            ...
            <table class="w-full text-left text-xs">
                <thead class="bg-slate-100 text-slate-700 uppercase text-[11px] font-bold border-b">
                    <tr>
                        ...
                        <th class="p-3.5">Tipe Workflow</th>
                        <th class="p-3.5">Dokumen</th>
                        <th class="p-3.5">Pemohon</th>
                        <th class="p-3.5">Posisi Layer Approval</th>
                        <th class="p-3.5">Status Akhir</th>
                        <th class="p-3.5 text-center">Audit Trail Log</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200">
                    @forelse($requests as $r)
                        <tr class="hover:bg-slate-50 transition">
                            <td class="p-3.5 font-mono font-bold text-slate-600">#REQ-{{ sprintf('%04d', $r->RequestID) }}</td>
                            <td class="p-3.5">
                                <span class="px-2.5 py-0.5 rounded font-bold text-[10px] uppercase
                            ...
                            <td class="p-3.5 text-slate-700 font-semibold">{{ $r->requester->name ?? 'User' }}</td>
                            ...
                            <td class="p-3.5 text-center">
                                <button @click="activeLogs = {{ json_encode($r->approvalLogs) }}; showLogModal = true"
                                        class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded-xl text-xs font-bold border border-slate-300">
                                    📜 Lihat Audit Log
                                </button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="p-8 text-center text-slate-500 font-medium">Belum ada riwayat pengajuan pengendalian dokumen.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <!-- Audit Log Modal -->
    <div x-show="showLogModal" x-cloak class="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 relative border border-slate-200">
            <button @click="showLogModal = false" class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 font-bold text-xl">&times;</button>
            ...
            <div class="space-y-3 max-h-60 overflow-y-auto">
                ...
                <template x-for="log in activeLogs" :key="log.LogID">
                    <div class="p-3 rounded-xl border text-xs" :class="log.Action === 'APPROVED' ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'">
